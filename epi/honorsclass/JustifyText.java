...
public class JustifyText {
    ...
    public static List<String> justifyText(String[] words, int L) {
        ...
        for (int i = 0; i < words.length; ++i) {
            ...
            if (lookaheadLineLength == L) {
                result.add(
                        joinALineWithBlank(words, currLineStart, i, i - currLineStart));
                currLineStart = i + 1;
                numWordsCurrLine = 0;
                currLineLength = 0;
        ...
        if (numWordsCurrLine > 0) {
            StringBuilder line = new StringBuilder(joinALineWithBlank(
    ...
    // Joins strings in words.subList(start, end + 1) with numBlanks blanks spread
    // evenly.
    private static String joinALineWithBlank(String[] words, int start, int end,
                                             int numBlanks) {
        ...
        for (int i = start; i < end; ++i) {
            line.append(words[i]);
            --numWordsCurrLine;
            int numCurrBlank = (int) Math.ceil((double) numBlanks / numWordsCurrLine);
